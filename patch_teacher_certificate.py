...
# Add activeAward state
content = content.replace(
    "const [activeDoc, setActiveDoc] = useState<any>(null);",
    "const [activeDoc, setActiveDoc] = useState<any>(null);\n  const [activeAward, setActiveAward] = useState<any>(null);"
)

# Update the cards to be clickable
old_card = """              {data.awards.map((award: any) => (
...
new_card = """              {data.awards.map((award: any) => (
                <div key={award.id} 
...
# Add a 'View Certificate' hint in the card
old_card_content = """                  <p className="text-amber-800/80 text-sm leading-relaxed relative z-10">
                    {award.reason}
                  </p>
                </div>"""

new_card_content = """                  <p className="text-amber-800/80 text-sm leading-relaxed relative z-10 line-clamp-2">
                    {award.reason}
                  </p>
...
# Add the Certificate Modal at the end, right before the closing div of the main container
certificate_modal = """
      {/* Certificate Modal */}
      {activeAward && (
                ...
                <div className="border border-amber-300 relative bg-white px-6 py-12 sm:px-16 sm:py-20 text-center overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    ...
                    <p className="text-slate-700 text-lg sm:text-xl font-medium mb-12 max-w-3xl leading-relaxed">
                      ...
                      <br/>
                      <span className="block mt-4 text-amber-800 font-bold bg-amber-50 py-3 px-6 rounded-xl border border-amber-100">{activeAward.reason}</span>
                    </p>
